...
async function handleSubmit(e) {
    e.preventDefault();
    (0, _pixabayApi.options).params.q = searchInputEl.value.trim();
    (0, _pixabayApi.options).params.page = 1;
    if ((0, _pixabayApi.options).params.q === "") return;
    galleryEl.innerHTML = "";
    reachEnd = false;
    try {
        const res = await (0, _axiosDefault.default).get((0, _pixabayApi.BASE_URL), (0, _pixabayApi.options));
        totalHits = res.data.totalHits;
        const hits = res.data.hits;
        // console.log(hits);
        if (totalHits === 0) (0, _notiflix.Notify).failure(`Sorry, there are no images matching your search query. Please try again.`);
        else (0, _notiflix.Notify).success(`Hooray! We found ${totalHits} images.`);
        searchFormEl.value = "";
        renderGallery(hits);
    } catch (e1) {
        (0, _notiflix.Notify).failure(e1);
    }
}
async function loadMore() {
    ...
    try {
        ...
        (0, _notiflix.Notify).failure(e);
    }
}
function handleScroll() {
    const { scrollTop , scrollHeight , clientHeight  } = document.documentElement;
    if (scrollTop + clientHeight >= scrollHeight) {
        if (totalHits > 0 && (0, _pixabayApi.options).params.page * (0, _pixabayApi.options).params.per_page >= totalHits) {
...
},{"./pixabay-api":"4M0T7","axios":"jo6P5","notiflix":"fuMq1","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"jo6P5":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{}],"kaxSc":[function() {},{}]},["bFOAz","dV6cC"], "dV6cC", "parcelRequirea610")
